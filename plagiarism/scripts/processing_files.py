...
def file_extension_call(file: str) -> list:
    ...
    if extension:
        if extension == '.pdf':
            return get_words_from_ocr_file(file)
        if extension == '.docx':
...
def get_words_from_ocr_file(pdf_path: str) -> list:
    """ Return list of words from pdf file at specified path """
    
    images = convert_from_path(pdf_path, poppler_path = r"C:\\Users\\bhatd\\Downloads\\Release-22.04.0-0\\poppler-22.04.0\\Library\\bin" )
    data = ""
